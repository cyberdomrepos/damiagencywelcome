...
export default function MusicCarouselNew({
  tracks,
  autoplay = false,
  interval = 4500,
  variant = "large",
  ...
  const startProgressLoop = useCallback(
    (i: number, buffer: AudioBuffer, canvas: HTMLCanvasElement) => {
      ...
      const loop = () => {
        ...
        // update time state for UI (avoids reading refs during render)
        setTimes((prev) => {
          const next = prev.slice();
          next[i] = {
            current: audio?.currentTime || 0,
            total: audio?.duration || next[i]?.total || 0,
          };
          return next;
        });
    ...
    (i: number) => {
      ...
      const loop = () => {
        try {
          ...
          const audioForTimes = audioRefs.current[i];
          setTimes((prev) => {
            const next = prev.slice();
            next[i] = {
              current: audioForTimes?.currentTime || 0,
              total: audioForTimes?.duration || next[i]?.total || 0,
            };
            return next;
          });
        } catch {
  ...
  useEffect(() => {
    ...
    audioRefs.current.forEach((a, i) => {
      ...
      const onTimeUpdate = () => {
        // update per-track time state
        setTimes((prev) => {
          const next = prev.slice();
          next[i] = {
            current: a.currentTime || 0,
            total: a.duration || next[i]?.total || 0,
          };
          return next;
        });
      };
  ...
  // `times` is initialized from `tracks` on first render; dynamic resizing
  // of `tracks` is uncommon for this component. If needed, we can add
  // a controlled prop to reset times from outside.

  // Start waveform/progress loops when a track begins playing.
  // Move side-effects out of render/ref callbacks and into this effect.
  useEffect(() => {
    ...
    if (canvas) {
      if (buffer) {
        startProgressLoop(i, buffer, canvas);
      } else {
        const analyser = analyserRefs.current[i] || ensureAnalyserFor(i);
              ...
              >
                ...
                >
                  ...
                  <div className="absolute inset-x-6 md:inset-x-12 bottom-24 md:bottom-28 z-10 bg-black/40 rounded-lg p-2">
                    <canvas
                      ref={(el) => {
                        // only assign ref here — avoid running side-effects inside render
                  ...
                  <div className="absolute bottom-0 left-0 right-0 z-10 bg-linear-to-t from-black/80 via-black/60 to-transparent pt-12 pb-4 px-6 md:px-8">
                    ...
                    <div className="flex items-end justify-between mb-3">
                      ...
                      <div className="text-base md:text-lg font-mono text-white drop-shadow-lg bg-black/50 px-4 py-2 rounded-lg">
                        {(() => {
                          const current = Math.floor(times[i]?.current || 0);
                          const total = Math.floor(times[i]?.total || 0);
                    ...
                    <div
                      ...
                      onClick={(e) => {
                        ...
                        // Update time state immediately after seeking
                        setTimes((prev) => {
                          const next = prev.slice();
                          next[i] = {
                            current: audio.currentTime || 0,
